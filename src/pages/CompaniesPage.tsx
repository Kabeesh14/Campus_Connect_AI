      ...
      <Reveal delay={0.05}>
        <div className="gradient-border overflow-hidden rounded-3xl">
          <div className="relative h-48 overflow-hidden sm:h-64">
            <img src={company.cover} alt={company.name} className="h-full w-full object-cover" />
          ...
          <div className="px-6 pb-6 sm:px-8">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
              <div className="flex items-end gap-4">
                <img src={company.logo} alt={company.name} className="h-20 w-20 rounded-2xl object-cover ring-4 ring-[rgb(var(--card))] sm:-mt-10" />
                <div>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              <Badge variant="primary"><Star size={12} className="fill-warning text-warning" /> {company.rating} rating</Badge>
              <Badge variant="accent">{company.openRoles} open roles</Badge>
              <Badge variant="warning"><Clock size={12} /> {company.deadlineDays} days to deadline</Badge>
            </div>
          </div>
        </div>
      </Reveal>
          ...
          {tab === 'overview' && (
            <GlassCard>
              ...
              <p className="mt-3 text-sm leading-relaxed text-soft">
                {company.name} is a leading {company.industry.toLowerCase()} company with {company.employees.toLocaleString()}+ employees worldwide. They are currently hiring for {company.openRoles} roles with an eligibility of {company.eligibility}. The average package is {company.salary} and the application deadline is in {company.deadlineDays} days.
              </p>
              <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div className="rounded-2xl border border-base bg-soft/40 p-4">
                  ...
                  <p className="mt-1 font-semibold">{company.eligibility}</p>
                </div>
                <div className="rounded-2xl border border-base bg-soft/40 p-4">
